...
def conv1d(x, n_filters, n_size, stride=1, nl='relu', name='conv1d'):
  ...
  with tf.compat.v1.variable_scope(name):
    ...
    with tf.compat.v1.name_scope('weights'):
      W = tf.compat.v1.get_variable('W', shape=[n_size, n_input_chan, n_filters],
    ...
    with tf.compat.v1.name_scope('biases'):
      b = tf.compat.v1.get_variable('b', [n_filters], initializer=tf.compat.v1.constant_initializer(0.))
...
def deconv1d(x, r, n_chan, n_in_dim, n_in_chan, name='deconv1d'):
  ...
  with tf.compat.v1.variable_scope(name):
    ...
    W = tf.compat.v1.get_variable('W', shape=[1, r, n_chan, n_in_chan],
                        initializer=tf.compat.v1.random_normal_initializer(stddev=1e-3))
    b = tf.compat.v1.get_variable('b', [n_chan], initializer=tf.compat.v1.constant_initializer(0.))
