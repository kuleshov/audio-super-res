...
class Model(object):
	...
	def create_train_op(self, X, Y, alpha):
		...
		optimizer_vars = [ v for v in tf.compat.v1.global_variables() if 'optimizer/' in v.name or 'Adam' in v.name ]
...
#		print('DataSet X', X_train.size)
#		print('DataSet Y', Y_train.size)
		...
		while train_data.epochs_completed < n_epoch:
			...
			print('Step', step)
			print('epoch', epoch)
			print('epochs_completed:', train_data.epochs_completed)
			print('Step batch', len(batch[0]))
			...
			if train_data.epochs_completed > epoch:
				...
				if(calc_full_snr and train_data.epochs_completed % 1 == 0 and grocery == 'false'):
						...
						if piano == "true":
								...
								s2 = "-piano-interp-val." + str(r) + '.16000.-1.4096.0.1'
						elif speaker == "single":
	...
	def train(self, feed_dict):
		...
		#print(self.loss)
		#print(feed_dict['X:0'])
		#print(tf.size(self.train_op))

		# for key in feed_dict.keys():
		# 	print(type(feed_dict[key]))
		# 	if type(feed_dict[key]) == "<class 'numpy.ndarray'>":
		# 		print(feed_dict[key].shape)

		# THIS LOOKS LIKE IT'S TRYING TO ACHEIVE A "LOSS" objective using the "TRAIN OPTIMIZER" on the FEED_DICT Data
		# Both X and Y are [64,8192,1], which means somehow the "train_op" and "loss" here are causing a misalignment with dimensions
		# [64,40960,1] vs. [64,8192,1]
	...
	def load_batch(self, batch, alpha=1, train=True):
		...
		k_tensors = [n for n in g.as_graph_def().node if n.name == 'keras_learning_phase']
