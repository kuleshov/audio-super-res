...
for model in models:
    for size in sizes:
        for r in rs:
            ...
            trainstr = '../data/vctk/speaker1/vctk-speaker1-train.' + \
                r+'.16000.' + size + '.' + stride + '.h5'
            valstr = '../data/vctk/speaker1/vctk-speaker1-val.' + \
                r+'.16000.' + size + '.' + stride + '.h5'
            command = 'python run.py train --train ' + trainstr + ' --val ' + valstr + ' -e 50 --batch-size 8 --lr 3e-4 --layers 4 --piano false  --logname full_end_normalized_parmas_ps' + \
                pool_size+'.s'+pool_stride+'-'+model+'.lr0.00300.1.g4.b32.d'+size+'.r'+r+' --model ' + \
                model + ' --r ' + r + ' --pool_size ' + pool_size + ' --strides ' + pool_stride
            print(command)
